...
namespace Halia
{
	...
	//! Defines a color or alpha blend-mode for a texture-stage
	struct BlendMode
	{
	...
	//! A class representing a single texture stage in a material
	class TextureStage
	{
	public:
		TextureStage( Texture* tex, BlendMode cm, BlendMode am, const Matrix4& t )
			: texture( tex ), colormode( cm ), alphamode( am ), transform( t ), uses_transform( true )
		{
		};

		TextureStage( Texture* tex, BlendMode cm, BlendMode am )
			: texture( tex ), colormode( cm ), alphamode( am ), uses_transform( false )
		{
		};

		TextureStage( Texture* tex, BlendMode m, const Matrix4& t )
			: texture( tex ), colormode( m ), alphamode( m ), transform( t ), uses_transform( true )
		{
		};

		TextureStage( Texture* tex, BlendMode m )
